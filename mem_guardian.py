...
Heavy offline compute (the mm_dream_reconstruction binaries and the Python QA)
can balloon under a high-statistics beam run. If system memory is exhausted the
machine thrashes/freezes and can take the live DAQ down with it. This daemon
...
the DAQ (RunCtrl, hv_control, dream_daq_control, daq_control, flask), never
itself. The killed job's watcher simply retries it later.
...
only signals processes owned by the same user.
...
DEFAULTS = {
    # 62 GB machine: kill a runaway well before the kernel OOM/freeze, keeping
    # plenty of headroom for the DAQ. Tune down on a smaller box.
    "kill_avail_mb": 4000,
    "warn_avail_mb": 8000,
    "poll_s": 2.0,
    "recover_s": 20.0,
    # Only these (heavy, restartable) compute processes may be killed.
    "killable": [
        "/decode", "analyze_waveforms", "combine_feus_hits",  # reconstruction
        "detector_qa.py", "pedestal_qa", "beam_spectra", "tag_probe",  # analysis/QA
    ],
    # Safety veto: never kill the live DAQ or the guardian itself, even if a
    # substring above happened to match part of their command line.
    "protect": [
        "RunCtrl", "hv_control.py", "dream_daq_control.py", "daq_control.py",
        "backup_watcher.py", "beam_watcher.py", "start_flask", "flask",
        "mem_guardian.py",
    ],
}
